...
def area_transferencia(request):
    if header.validators.verficarAcessoTransferencia(request):
        return render(request, 'utilizador/erro-acesso.html', {})

    form = Nip_Form(request.POST or None)
    dados = {'fotos': request.session['salakiaku'],
             'form': form, 'transferencias': MENU_TRANSFERENCIA}
...
def aprovar_transferencia(request, id):
    ...
    if request.method == 'POST':
        ...
        ap.save()
        sweetify.success(
            request, 'Foi Enviado uma Notificação por E-mail <br> com Sucesso!....', button='Ok', timer='3900')
...
def adicionar_troca(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            try:
                id_1 = header.views_core.retorna_numero_agente(
                    form.cleaned_data.get('bi1'))
                id_2 = header.views_core.retorna_numero_agente(
                    form.cleaned_data.get('bi2'))
                troca = form.save(commit=False)
                troca.origem_segundo = form.cleaned_data.get(
                    'destino_primeiro')
                troca.destino_segundo = form.cleaned_data.get(
                    'origem_primeiro')
                troca.primeiro_agente_id = id_1
                troca.segundo_agente_id = id_2
                troca.save()
                sweetify.success(
                    request, 'Troca realizada com sucesso!....', button='Ok', timer='3500')
                return HttpResponseRedirect(reverse('transferencia:area-transferencia'))
            except Exception as e:
                print(e)

    context = {'form': form,
               'fotos': request.session['salakiaku'], 'transferencias': MENU_TRANSFERENCIA}
...
def registar_transferencia(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            try:
                ids = header.views_core.retorna_numero_agente(
                    form.cleaned_data.get('bi'))
                desp = form.save(commit=False)
                desp.agente_id = ids
                desp.save()
                sweetify.success(
                    request, 'Dados Adicionado com sucesso!....', button='Ok', timer='3500')
...
def atualizar_pedido_transferencia(request, id):
    ...
    try:
        if request.method == 'POST':
            if form.is_valid():
                ...
                template = TEMPLATE_UTILIZADOR['perfil_rota']
                sweetify.success(
                    request, 'Dados atualizado com sucesso!....', button='Ok', timer='3500')
                return HttpResponseRedirect(reverse('transferencia:area-transferencia'))
    except Exception as e:
    ...
    context = {'form': form, 'form2': pessoa, 'pedido': pedido,
               'fotos': request.session['salakiaku'], 'transferencias': MENU_TRANSFERENCIA}
    template = TEMPLATE_TRANSFERENCIA['transferencia']
    return render(request, template, context)
        ...
        if request.method == 'POST':
            if form.is_valid():
                ...
                dados = {'form': form, 'listar': lista,
                         'transferencias': MENU_TRANSFERENCIA}
...
def listar_pedido_transferencia(request):
    ...
    lista = Transferencia.objects.select_related(
        'agente').all().filter(situacao="Espera")
    context = {'lista': lista,
               'fotos': request.session['salakiaku'], 'transferencias': MENU_TRANSFERENCIA}
...
def listar_agentes_transferido(request):
    ...
    lista = Transferencia.objects.select_related(
        'agente').all().filter(situacao="Aprovado")
    context = {'lista': lista,
               'fotos': request.session['salakiaku'], 'transferencias': MENU_TRANSFERENCIA}
        ...
        if value is not None and len(value) > 0:
            try:
                ...
                if agen.nip is not None:
                    ...
                    context = {
                        'lista': tras, 'fotos': request.session['salakiaku'], 'transferencias': MENU_TRANSFERENCIA}
                    template = TEMPLATE_TRANSFERENCIA['listar_pedido']
                    return render(request, template, context)
            except Exception as e:
                try:
                    ...
                    sweetify.warning(
                        request, 'Não existem Transferencia para este agente!....', button='Ok', timer='3800')
                    return HttpResponseRedirect(reverse('transferencia:area-transferencia'))
...
def emitir_guia_transferencia(request, id):
    ...
    if request.method == 'POST' or value > 0:
        if form.is_valid():
            value = header.views_core.retorna_numero_agente(
                request.POST['nip'])
            print(value)
        ...
        try:
            ...
            doc = SimpleDocTemplate(response, pagesize=letter, rightMargin=55,
                                    leftMargin=55, topMargin=30, bottomMargin=75, author="Ismael")
            ...
            centro = ParagraphStyle(
                name='top', alignment=TA_CENTER, fontName="Times-Roman")
            negrito = ParagraphStyle(
                name='top', alignment=TA_CENTER, fontName="Times-Bold")
            direito = ParagraphStyle(
                name='top', alignment=TA_RIGHT, fontName="Times-Bold")
            ...
            DESPACHO = """<font size=12>Of. Nr. %s /GAB CGPN/328-L2GCG/2019</font>""" % (
                str(resp.numero_guia))
            ...
            sweetify.error(request, 'Não possui guia de transferencia',
                           position='top-end',   persistent='OK')
