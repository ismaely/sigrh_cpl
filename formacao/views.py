...
def area_formacao(request):
    if header.validators.verficarAcessoFormacao(request):
...
def visualizar_informacao_conclusao(request, id=None):
    ...
    dados = {'lista': lista, 'formacao': formacao,
             'formacaoes': MENU_FORMACAO, 'fotos': request.session['salakiaku']}
...
def marca_precensa_selecionado(request, id):
    ...
    try:
        ...
        presenca = Presenca.objects.create(
            agente_id=id, selecionado_id=sels.id, data=DATE_FORMAT)
        sweetify.success(
            request, 'Formação confirmada com sucesso...', button='Ok', timer='3300')
    except Exception as e:
        sweetify.error(
            request, 'Não foi possivel, agente não existe...', button='Ok', timer='3300')
        print(e)
...
def buscar_selecionado_atualizar(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            sweetify.error(request, 'Não existe formação..',
                           button='Ok', timer='4000')
...
def listar_todos_terminaram_formacao(request):
    ...
    try:
        ...
        raise Http404(
            " erro na listagem de todos que  ja terminaram a formacao %s " % (e))
...
def eliminar_documento(request, id=None):
    ...
    if id is not None:
        ...
        sweetify.success(
            request, 'Documento eliminado com sucesso!....', button='Ok', timer='3500')
        return HttpResponseRedirect(reverse('formacao:listar-docs'))
    else:
        sweetify.error(request, 'Falha Não foi possivel!....',
                       button='Ok', timer='3500')
        return HttpResponseRedirect(reverse('formacao:listar-docs'))
...
def atualizar_selecionado_formacao(request, id):
    ...
    form = SelecionarForm(request.POST or None, instance=selec, initial={
                          'bi': selec.agente.pessoa.bi})
    if request.method == 'POST':
        if form.is_valid() and header.validators.consultar_bi_existe(request.POST['bi']):
            agen = header.views_core.retorna_numero_agente(request.POST['bi'])
            desp = form.save(commit=False)
            desp.agente_id = agen
            desp.save()
            sweetify.success(
                request, 'Daods atualizado com sucesso!....', button='Ok', timer='3300')
            return HttpResponseRedirect(reverse('formacao:area-formacao'))
        else:
    ...
    dados = {'form': form, 'form2': pessoa, 'forma': selec,
             'formacaoes': MENU_FORMACAO, 'fotos': request.session['salakiaku']}
...
def atualizar_conclusao_formacao(request, id):
    ...
    form = FormacaoConlusao_Form(request.POST or None, instance=conclui, initial={
                                 'bi': conclui.agente.pessoa.bi})
    if request.method == 'POST':
        if form.is_valid() and header.validators.consultar_bi_existe(request.POST['bi']):
            agen = header.views_core.retorna_numero_agente(request.POST['bi'])
            desp = form.save(commit=False)
            ...
            desp.save()
            sweetify.success(
                request, 'Dados atualizado com sucesso....', button='Ok', timer='3300')
            return HttpResponseRedirect(reverse('formacao:area-formacao'))
        else:
    ...
    pessoa = PessoaForm(request.POST or None, instance=pes)
    dados = {'form': form, 'form2': pessoa, 'forma': conclui,
             'fotos': request.session['salakiaku'], 'formacaoes': MENU_FORMACAO}
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if header.validators.validar_conclusao_formacao(request):
                ...
                seleciona.delete()
                sweetify.success(
                    request, 'Conclusão da formação adicionado com sucesso!....', button='Ok', timer='3300')
                return HttpResponseRedirect(reverse('formacao:area-formacao'))
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if header.validators.validar_selecionar_formacao(request) and header.validators.verficar_falecimento(request, agen):
                ...
                query = Selecionado_formacao.objects.select_related('agente').get(id=desp.id)
                sweetify.success(
                    request, 'Agente adicionado com sucesso!....', button='Ok', timer='3300')
                return render(request, 'formacao/informacao.html', {'fotos': request.session['salakiaku'], 'lista': query, 'formacaoes': MENU_FORMACAO})
                #return HttpResponseRedirect(reverse('formacao:area-formacao'))
    ...
    if request.method == 'POST':
        ...
        if value is not None:
            ...
            try:
                ...
                context = {'formacao': formacao,
                           'lista': orgao, 'formacaoes': MENU_FORMACAO}
                template = TEMPLATE_FORMACAO['consultar']
                return render(request, template, context)
            except Exception as e:
                sweetify.warning(
                    request, 'Não existe agente cadastrado com esse numero no sistema!....', button='Ok', timer='3900')
                return HttpResponseRedirect(reverse('formacao:area-formacao'))
...
def ficha_formacao(request, id=None):
    ...
    try:
        ...
        lista_dados = Formacao_conclusao.objects.select_related(
            'agente').filter(agente_id=id)
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'inline; filename="ficha_pessoal.pdf"'

        buffer, p, logo, fotofardado, logo_tabela, table1, table2, table3, table4, table5, table6, estilosB, style = cabecarioFicha(
            id)
        ...
        for formacao in lista_dados:
            dados_formacao = [str(formacao.curso), str(formacao.data_conclusao), str(
                formacao.dispacho), str(formacao.aproveitamento)]

        dados_formacao2 = []
        for formacao in lista_dados:
            dados_formacao2 = [str(formacao.pais), str(
                formacao.instituicao), str(formacao.razao_posse)]
        ...
        table = Table(discip2, colWidths=[
                      4.4 * cm, 11.4 * cm, 3.4 * cm, 1.1 * cm])
        table.setStyle(TableStyle([
            ('GRID', (0, 0), (6, -1), 1.3,  colors.black),
            ('LINEBELOW', (0, 0), (-1, 0), 1.3, colors.black),
            ('BACKGROUND', (0, 0), (-1, 0), colors.white)
        ...
        sweetify.error(request, 'Não existe dados do agente!....',
                       button='Ok', timer='3500')
        return HttpResponseRedirect(reverse('formacao:area-formacao'))
